...
async function get_users_filtered(custom_fields = {}){
  let default_fields = { uid: 1 }
  Object.assign(default_fields, custom_fields)
  let data = await dbsrv.mongo_groups().find({}).project(default_fields).toArray();
  return data
}
...
function get_user_home(user) {
    ...
    if (
        CONFIG.general.never_update_user_home &&
        user.home &&
        user.home.length > user.uid.length &&
        user.home != new_home
    ) {
        new_home = user.home;
    }
...
async function create_user(user, action_owner = 'auto') {
    ...
    await dbsrv
        ...
        .insertOne({
            owner: action_owner,
            date: new Date().getTime(),
            action: 'user registration ' + user.uid,
            logs: []
        });
...
async function remove_user_from_group(uid, secgroup, action_owner = 'auto') {
    ...
    await dbsrv
        ...
        .insertOne({
            owner: action_owner,
            date: new Date().getTime(),
            action: 'remove user ' + uid + ' from secondary  group ' + secgroup,
            logs: [user.uid + '.' + fid + '.update']
        });
    let users_in_group = await dbsrv
        .mongo_users()
        .find({ $or: [{ secondarygroups: secgroup }, { group: secgroup }] })
        .toArray();
    ...
    try {
        ...
        await dbsrv
            ...
            .insertOne({
                owner: action_owner,
                date: new Date().getTime(),
                action: 'add user ' + uid + ' to secondary  group ' + secgroup,
                logs: [created_file]
            });
    } catch (error) {
        logger.error('Group Change Failed for: ' + user.uid, error);
...
async function remove_user_from_project(oldproject, uid, action_owner = 'auto', action_owner_is_admin = false, force = false) {
    ...
    await dbsrv
        ...
        .insertOne({
            owner: action_owner,
            date: new Date().getTime(),
            action: 'remove user ' + uid + ' from project ' + oldproject,
            logs: []
        });
    ...
    try {
        ...
        await dbsrv
            ...
            .insertOne({
                owner: action_owner,
                date: new Date().getTime(),
                action: 'add user ' + uid + ' to project ' + newproject,
                logs: [created_file]
            });
    } catch (error) {
        logger.error(error);
    ...
    if (CONFIG.project === undefined || CONFIG.project.enable_group) {
        try {
            ...
            if (!error.code || error.code >= 300) {
                throw {
                    code: 500,
                    message: 'Add User to group;' + project.group + ' Failed for project: ' + project.id
                };
            }
        }
    }
