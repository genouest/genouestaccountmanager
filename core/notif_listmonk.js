...
async function join_list(user, lists) {
    ...
    try {
        ...
        // ask for optin
        await axios.post(
            CONFIG.listmonk.host + `/api/subscribers/${user.id}/optin`,
            {},
            {
                auth: auth,
                headers: {
        ...
        if (ok) {
            dbsrv
                ...
                .insertOne({
                    ...
                    action: 'add ' + email + ' to mailing list ' + CONFIG.listmonk.optin.join(','),
                    logs: []
                });
        } else {
            dbsrv
                ...
                .insertOne({
                    date: new Date().getTime(),
                    action: 'subscription error ' + email + ' to mailing list ' + CONFIG.listmonk.optin.join(','),
                    logs: []
                });
        }
    },
    ...
    remove: async function (email, sendmail = true) {
        ...
        if (ok) {
            dbsrv
                ...
                .insertOne({
                    date: new Date().getTime(),
                    action: 'remove ' + email + ' from mailing list ' + CONFIG.listmonk.optout.join(','),
                    logs: []
                });
        } else {
            dbsrv
                ...
                .insertOne({
                    date: new Date().getTime(),
                    action: 'unsubscribe error ' + email + ' from mailing list ' + CONFIG.listmonk.optout.join(','),
                    logs: []
                });
        }
    },
        ...
        if (olduser) {
            ...
            if (!ok) {
                await dbsrv
                    ...
                    .insertOne({
                        date: new Date().getTime(),
                        action: 'subscription update error with ' + oldemail + 'in mailing list',
                        logs: []
                    });
            ...
            if (ok) {
                dbsrv
                    ...
                    .insertOne({
                        date: new Date().getTime(),
                        action: 'add ' + newemail + ' to mailing list ' + CONFIG.listmonk.optin.join(','),
                        logs: []
                    });
            } else {
                dbsrv
                    ...
                    .insertOne({
                        date: new Date().getTime(),
                        action: 'subscription error ' + newemail + ' to mailing list ' + CONFIG.listmonk.optin.join(','),
                        logs: []
                    });
            }
        }
