        ...
        client.search('ou=groups,' + CONFIG.ldap.dn, opts, function (err, res) {
            ...
            res.on('end', function (result) {
                ...
                if (group_dn_list.length > 1) {
                    ...
                    logger.warn(
                        'switch to default value, you should check your ldap database and scripts logs results...'
                    );
                    dbsrv
                        .mongo_events()
                        .insertOne({
                            owner: CONFIG.general.admin[0],
                            date: new Date().getTime(),
                            action: '[error] find duplicate group dn for ' + group,
                            status: -1
                        });
        ...
        return new Promise(function (resolve, reject) {
            ...
            let prm_add = group_add.map((group) => {
                return get_group_dn(group)
                    ...
                    .catch((error) => {
                        logger.error('Get Dn to Add Failed for: ' + group, error);
                    });
            });
            ...
            let prm_remove = group_remove.map((group) => {
                return get_group_dn(group)
                    ...
                    .catch((error) => {
                        logger.error('Get Dn to Remove Failed for: ' + group, error);
                    });
            });

            Promise.all(prm_add).then((group_add_dn) => {
                //logger.info( 'To add: ', group_add_dn);
                Promise.all(prm_remove).then((group_remove_dn) => {
                    // logger.info( 'To remove: ', group_remove_dn);

                    filer
                        .ldap_change_user_groups(user, group_add_dn, group_remove_dn, fid)
                        .then((created_file) => {
                            logger.info('File Created: ', created_file);
                            resolve();
                        })
