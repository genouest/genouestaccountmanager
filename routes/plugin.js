...
router.get('/plugin/:id/:user', async function (req, res) {
    ...
    if (req.params.user != user.uid && !user.is_admin) {
        return res.status(403).send({ message: 'Not authorized to get an other user data if not admin...' });
    }
...
router.post('/plugin/:id/:user', async function (req, res) {
    ...
    user.is_admin = isadmin;
    if (req.params.user != user.uid && !user.is_admin) {
        return res.status(403).send({ message: 'Not authorized to update an other user data if not admin...' });
    }
    ...
    plugins_modules[req.params.id].set_data(req.params.user, req.body, user.uid).then(
        ...
        function (err) {
            return res.status(400).send({ message: err }); // Maybe we should check if err is a string
        }
    );
});
